...
export function MarketplaceClient() {
  ...
  const buy = async (listing: Listing) => {
    ...
    try {
      ...
      if (allowance < listing.price) { setMessage('Confirm AQT spending approval in your wallet.'); const approvalHash = await writeContractAsync({ address: token, abi: gameTokenAbi, functionName: 'approve', args: [market, listing.price], chainId: avalancheFuji.id }); setHash(approvalHash); await publicClient.waitForTransactionReceipt({ hash: approvalHash }); }
      setMessage('Confirm the NFT purchase in your wallet.'); const nonce = await publicClient.getTransactionCount({ address, blockTag: 'pending' });
      const buyHash = await writeContractAsync({ address: market, abi: marketplaceAbi, functionName: 'buy', args: [listing.id], chainId: avalancheFuji.id, nonce }); setHash(buyHash); await publicClient.waitForTransactionReceipt({ hash: buyHash }); setState('success'); setMessage('NFT purchase completed successfully.'); await load();
