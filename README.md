...
쌍검사와 권격가는 대기, 걷기, 달리기, 점프·대시, 기본 공격, 스킬 시전, 피격, 사망 동작을 각각 애니메이션으로 재생합니다. 두 클래스 모두 Q/W/E/R/T 5개 스킬을 기본으로 사용할 수 있으며 R만 버프 스킬입니다. 쌍검사는 적의 뒤로 순간 이동하는 `그림자 역습`, 권격가는 거대한 주먹 투사체를 날리는 `거신권`을 포함합니다.
